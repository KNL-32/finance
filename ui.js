...
function generateId() {
  counter_id++;
  return "rec_" + String(counter_id).padStart(4, "0");
}
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  for (let i = 0; i < inputs.length; i++) {
    ...
    // When the user types or changes something
  ...
  document.getElementById("tbody").addEventListener("click", function (e) {
    ...
    if (e.target.classList.contains("delete-btn")) {
      transactionArr = transactionArr.filter((t) => t.id !== id);

      saveTransactions(transactionArr);
      renderApp();
      addtodashboard();
    }
    ...
    if (e.target.classList.contains("edit-btn")) {
      const transaction = transactionArr.find((t) => t.id === id);
  ...
  counter_id = transactionArr.length;
  ...
  form.addEventListener("submit", function (event) {
    ...
    const date = document.getElementById("date").value; // NEW VALIDATION USING YOUR REGEX

    if (!validateField("description", description)) {
      showMessage(
        "Please enter a valid description without extra spaces.",
        true,
      );
      return;
    } // Check your advanced rule for duplicate words

    if (patterns.duplicate.test(description)) {
      showMessage(
        "Please do not use duplicate words next to each other.",
        true,
      );
      return;
    }

    if (!validateField("amount", amount) || Number(amount) <= 0) {
      showMessage("Please enter a valid number amount greater than 0.", true);
      return;
    }

    if (!validateField("category", category) || category === "") {
      showMessage("Please choose a valid category.", true);
      return;
    }

    if (!validateField("date", date)) {
      showMessage("Please select a valid date in YYYY-MM-DD format.", true);
      return;
    }

    // Create transaction object and add to in-memory array
    if (editingId) {
      const transaction = transactionArr.find((t) => t.id === editingId);
...
function showTable(transactions) {
  ...
  <td>
  <button class="edit-btn" data-id="${transaction.id}" aria-label="Update transaction">✏️</button>
   <button class="delete-btn" data-id="${transaction.id}" aria-label="Delete transaction">🗑️</button>
  </td>
`;
